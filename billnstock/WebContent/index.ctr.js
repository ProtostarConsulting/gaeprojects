angular.module( "stockApp" ).controller( "indexCtr", function ( $scope, $rootScope, $window, $log, $q, $timeout, $mdToast, $mdBottomSheet, $state, $http, $location, $anchorScroll, $mdMedia, ajsCache, appEndpointSF ) {
	...
	$scope.showUpdateToast = function () {
		$mdToast.show( $mdToast.simple().content( 'Changes Saved Successfully.' ).position( "top" ).hideDelay( 3000 ) );
	};

	$scope.showAddToast = function () {
		$mdToast.show( $mdToast.simple().content( 'New Record Saved Successfully.' ).position( "top" ).hideDelay( 3000 ) );
	};

	$scope.showDelToast = function () {
		$mdToast.show( $mdToast.simple().content( 'Account  Deleted ...!' ).position( "top" ).hideDelay( 3000 ) );
	};
	...
	function getUserAuthTree () {
		...
		authService.getAuthorizationMasterEntity().then( function ( result ) {
			...
			var jsonUserAuthObject = angular.fromJson( $scope.curUser.authorizations );
			...
			if ( result && result.authorizations != undefined ) {

				authorizationMasterEntity.authorizations = result.authorizations;

				userAuthMasterEntity = authService.filterMasterAuthTree( authorizationMasterEntity, $scope.userAuthObject, userAuthMasterEntity );

				userAuthMasterEntity.authorizations.sort( function ( a, b ) {
					return ( parseFloat( a.orderNumber ) > parseFloat( b.orderNumber ) ) ? 1 : -1
				} );

				/*
				 * $log .debug("userAuthMasterEntity:" + angular .toJson(userAuthMasterEntity));
				 */

				var curUser = appEndpointSF.getLocalUserService().getLoggedinUser();
				curUser.userAuthMasterEntity = userAuthMasterEntity;
				appEndpointSF.getLocalUserService().saveLoggedInUser( curUser );
	...
	$scope.$on( 'event:google-plus-signin-success', function ( event, authResult ) {
		...
		// ID token instead.
		/*
		 * $log .debug('Name: ' + profile.getName()); $log.debug('Image URL: ' + profile.getImageUrl()); $log.debug('email_id: ' + profile.getEmail());
		 */
		$scope.googleUserDetails = profile.getName() + "<br>" + profile.getEmail()

		appEndpointSF.getUserService().getUserByEmailID( profile.getEmail() ).then( function ( loggedInUser ) {
	...
	$scope.signOut = function () {
		...
		if ( gapi.auth2 == undefined ) {
			$scope.curUser = null;
			$scope.curUser = appEndpointSF.getLocalUserService().logout();
	...
	$scope.initGAPI = function () {
		...
		$timeout( function () {
			appEndpointSF.loadAppGoogleServices( $q.defer() ).then( function () {
	...
	$scope.themeList = [
			'default', 'red', 'pink', 'purple', 'deep-purple', 'indigo', 'blue', 'light-blue', 'cyan', 'teal', 'green', 'light-green', 'lime', 'yellow', 'amber', 'orange', 'deep-orange', 'brown', 'grey', 'blue-grey'
	];
...
} ).controller( 'AppCtrl', function ( $scope, $timeout, $mdSidenav, $mdUtil, $log, $mdMedia ) {

	$scope.toggleMainMenuSwitch = $mdMedia( 'gt-md' );

	$scope.toggleMainMenu = function () {
		$scope.toggleMainMenuSwitch = !$scope.toggleMainMenuSwitch;
		$log.debug( "toggle left menu. $scope.toggleMainMenuSwitch:" + $scope.toggleMainMenuSwitch );
	}
