	...
	.controller(
		...
		function($scope, $window, $mdToast, $timeout, $mdSidenav,
			...
			function addSerialNumbersDialogController($scope, $mdDialog,
				curBusi, curUser, stockLineItem, callMe) {
				...
				var tempList = [];
				var existingList = [];
				if ($scope.stockLineItem.stockItemInstanceList && $scope.stockLineItem.stockItemInstanceList.length > 0) {
				...
				var stockService = appEndpointSF.getStockService();
				stockService.getStockItemInstancesList(stockLineItem.stockItem).then(
				...
				$scope.querySearch = function(query) {
					var diffArray = $scope.instanceListByStockItem.diff($scope.stockLineItem.stockItemInstanceList);
					var results = query ? diffArray.filter(createFilterFor(query)) : [];
					var deferred = $q.defer();
					$timeout(function() {
						deferred.resolve(results);
					}, Math.random() * 1000, false);
