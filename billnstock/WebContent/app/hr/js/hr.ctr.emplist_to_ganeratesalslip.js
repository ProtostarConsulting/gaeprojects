		...
		.controller(
				...
				function($scope, $window, $mdToast, $timeout, $mdSidenav,
					...
					$scope.getAllempsSalStruct = function() {
						...
						hrService
								.getAllempsSalStruct($scope.curUser.business.id)
								.then(
					...
					$scope.ganeratesalslip = function() {
						...
						for (i = 0; i < $scope.selected.length; i++) {
							$scope.selectedSalSlip = $scope.selected[i]
							...
							// $scope.salslip.empName=
							// $scope.selectedSalSlip.empName;
									...
									.then(
											function(gsalslip) {
												$scope.ganeratedsalslip
														.push(gsalslip.result);
												// $scope.showSimpleToast("salslip
												// ganareted");
												$log
														.debug("********%%%%%***********"
																+ angular
						...
						$scope
								.showSimpleToast("Salary slip generation process started. Please check salary slip list after sometime.");
						$state.go('hr.printgeneratesalslip', {
							sourceSate : "hr.generatesalslip",
							ganeratedsalslip : $scope.ganeratedsalslip
						});
