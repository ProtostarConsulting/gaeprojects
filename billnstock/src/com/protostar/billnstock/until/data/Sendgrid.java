...
public class Sendgrid {
	...
	protected String domain = "https://sendgrid.com/",
			endpoint = "api/mail.send.json", username, password;
	...
	public Sendgrid addTo(String email, String name) {
		if (this._useHeaders() == true) {
			String toAddress = (name.length() > 0) ? name + "<" + email + ">"
					: email;
	 ...
	 */
	public Sendgrid addSubstitution(String from_value, String[] to_values)
			throws JSONException {
	 ...
	 * @throws JSONException
	 */
	public Sendgrid setSections(JSONObject key_value_pairs)
			throws JSONException {
	 ...
	 * @throws JSONException
	 */
	public Sendgrid addSection(String from_value, String to_value)
			throws JSONException {
		if (false == this.header_list.has("section")) {
			this.header_list.put("section", new JSONObject());
		}
		((JSONObject) this.header_list.get("section"))
				.put(from_value, to_value);
	 ...
	 * @throws JSONException
	 */
	public Sendgrid addFilterSetting(String filter_name, String parameter_name,
			String parameter_value) throws JSONException {
		if (false == this.header_list.has("filters")) {
			this.header_list.put("filters", new JSONObject());
		}
		if (false == ((JSONObject) this.header_list.get("filters"))
				.has(filter_name)) {
			((JSONObject) this.header_list.get("filters")).put(filter_name,
					new JSONObject());
		}
		if (false == ((JSONObject) ((JSONObject) this.header_list
				.get("filters")).get(filter_name)).has("settings")) {
			((JSONObject) ((JSONObject) this.header_list.get("filters"))
					.get(filter_name)).put("settings", new JSONObject());
		}
		((JSONObject) ((JSONObject) ((JSONObject) this.header_list
				.get("filters")).get(filter_name)).get("settings")).put(
	...
	protected String _arrayToUrlPart(ArrayList<String> array, String token) {
		...
		for (int i = 0; i < array.size(); i++) {
			try {
				string += "&" + token + "[]="
						+ URLEncoder.encode(array.get(i), "UTF-8");
	...
	protected Map<String, String> _prepMessageData() throws JSONException {
		...
		if (this._useHeaders() == true) {
			...
			params.put("x-smtpapi",
					_escapeUnicode(this.getHeaders().toString()));
		} else {
	...
	public void send(WarningListener w) throws JSONException {
		...
		while (paramIterator.hasNext()) {
			...
			if (key.equals("to") && this.getTos().size() > 0) {
				if (this._useHeaders() == true) {
					try {
						requestParams.append("to="
								+ URLEncoder.encode(value, "UTF-8") + "&");
					} catch (UnsupportedEncodingException e) {
						e.printStackTrace();
					}
				} else {
					requestParams.append(this._arrayToUrlPart(this.getTos(),
							"to") + "&");
				}
			} else {
				if (key.equals("toname") && this.getToNames().size() > 0) {
					requestParams.append(this._arrayToUrlPart(
							this.getToNames(), "toname").substring(1)
							+ "&");
				} else {
					try {
		...
		if (this.getBccs().size() > 0) {
			request += "?"
					+ this._arrayToUrlPart(this.getBccs(), "bcc").substring(1);
		}
		try {
	...
	private Boolean _useHeaders() {
		if ((this._preferNotToUseHeaders() == true)
				&& (this._isHeadersRequired() == false)) {
			return false;
		} else {
			return true;
		}
