...
public class BaseService {
	...
	protected Object getEntityByItemNumber(Class c, int itemNumber) {
		...
		List list = ofy().load().type(c).filter("itemNumber", itemNumber)
				.list();
	...
	@SuppressWarnings("unchecked")
	protected EntityPagingInfo fetchEntityListByPaging(Long businessId,
			Class c, EntityPagingInfo pagingInfo) {
		Query ancestorQuery = ofy().load().type(c)
				.ancestor(Key.create(BusinessEntity.class, businessId))
				.order("-itemNumber");

		int totalCount = ancestorQuery.count();

		if (pagingInfo.getWebSafeCursorString() != null)
			ancestorQuery = ancestorQuery.startAt(Cursor
					.fromWebSafeString(pagingInfo.getWebSafeCursorString()));

		QueryResultIterator iterator = ancestorQuery.limit(
				pagingInfo.getLimit()).iterator();
